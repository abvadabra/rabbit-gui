...
public class JsonLayoutParser implements LayoutParser {
    ...
    private LayoutShow from(JsonElement json){
        ...
        String title = parseTitle(layout);
    ...
    private List<LayoutComponentWrapper> wrapComponents(JsonArray components){
        return Lists.newArrayList(components.iterator())
                .stream()
                .map(element -> wrap(element.getAsJsonObject()))
    ...
    private LayoutComponentWrapper wrap(JsonObject component){
        try{
            ...
            Class<? extends IGui> componentType = (Class<? extends IGui>) Class.forName(layoutFields.remove("type").getAsString());
